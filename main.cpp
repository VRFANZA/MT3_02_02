...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (IsCollision(s1, plane)) {
			DrawSphere(s1.center, s1.radius, viewProjectionMatrix, viewportMatrix, 0xFF0000FF);
		} else {
			DrawSphere(s1.center, s1.radius, viewProjectionMatrix, viewportMatrix, 0xFFFFFFFF);
		}

		DrawPlane(plane, viewProjectionMatrix, viewportMatrix, 0x000000FF);

		ImGui::Begin("Sphere s1 Controller");
		ImGui::SetWindowSize(ImVec2(400, 300)); // 幅400, 高さ300（お好みで変更）
		ImGui::SliderFloat3("s1.center", &s1.center.x, -5.0f, 5.0f);
		ImGui::SliderFloat("s1.radius", &s1.radius, 0.1f, 5.0f);
		ImGui::End();
...
{
	float distance = (
		plane.normal.x * s1.center.x +
		plane.normal.y * s1.center.y +
		plane.normal.z * s1.center.z) - plane.distance;
